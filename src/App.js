...
class App extends Component {
  ...
  onClickImage = thumbnailUrl => {
    ...
    const {imageUrl} = filteredImagesList[0]
  ...
  render() {
    const {isTrue, score, time, activeTabId, activeImageUrl} = this.state
        ...
        <div className="content-container">
          {!isTrue && (
            <div className="tab-item-container">
              <img src={activeImageUrl} alt="match" className="big-img" />
              <ul className="tab-list">
                {tabsList.map(eachValue => (
                  <li className="tab-item" key={eachValue.tabId}>
                    <button
                      type="button"
                      className={`tab-btn ${
                        activeTabId === eachValue.tabId ? 'active-btn' : ''
                      }`}
              ...
              <div className="thumbnail-container">
                <ul className="thumbnail-list">
                  {thumbnailList.map(eachValue => (
                    <li className="thumbnail-item" key={eachValue.id}>
                      <button
                        type="button"
                        className="thumbnail-btn"
                        onClick={() =>
                          this.onClickImage(eachValue.thumbnailUrl)
                        }
                      >
                        <img
